	...
	<div class="study">
		...
		<div class="container overflow">
			...
			<ul class="overflow" v-show="show">
				<li v-for="(item,index) in Treedata_y" :key="index">
					<van-collapse v-model="activeNames" @change="change(activeNames,item.ID,index)" :id="item.ID">
						<van-collapse-item :name="index">
							...
							<div v-show="showList">
								<van-cell is-link v-for="(list,index) in Treedata_r" :key="index" :id="list.ID" @click="TreedataR($event,list.ID,index,list.NAME)">
									<img src="../../assets/images/user_icon/icon_g@2x (4).png" alt="" width='12' :id="list.ID" v-if="list.NAME=='桥涵专业'" @click="TreedataR($event,item.ID,index,list.NAME)">
									<img src="../../assets/images/user_icon/icon_g@2x (2).png" alt="" width='12' :id="list.ID" v-if="list.NAME=='轨道专业'" @click="TreedataR($event,item.ID,index,list.NAME)">
									<img src="../../assets/images/user_icon/icon_g@2x (1).png" alt="" width='12' :id="list.ID" v-if="list.NAME=='路基专业'" @click="TreedataR($event,item.ID,index,list.NAME)">
									&nbsp;&nbsp;{{list.NAME}}
									<span style="color: #aaa;float: right;":id="list.ID" @click="TreedataR($event,list.ID,index,item.NAME)">已完成</span>
									<span style="color: #69966F;float: right;":id="list.ID" @click="TreedataR($event,list.ID,index,item.NAME)">22% </span>
								</van-cell>
...
<script>
	...
	export default {
		...
		created() {
			...
			localStorage.setItem("intellgent_option1_value", Number(this.$route.query.ValueId))
			if (this.value1 == localStorage.getItem("intellgent_option1_value")) {
				this.show = true;

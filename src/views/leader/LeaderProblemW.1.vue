	...
	<div id="examrecord">
		...
		<div class="container">
			...
			<ul class="container_list container_lists">
				...
				<li class="overflow" v-for="(image, index) in quesPic" :key="index">
					<div><img :src="image[0]" alt="" v-if="image[0]!=''"></div>
					<div><img :src="image[1]" alt="" v-if="image[1]!=''"></div>
					<div><img :src="image[2]" alt="" v-if="image[2]!=''"></div>
					<div><img :src="image[3]" alt="" v-if="image[3]!=''"></div>
				</li>
			</ul>
			<!-- 驳回修改按钮显示 -->
			<div>
				...
				ajax.getW('/api/safety/selectSafetyInfoById?id='+that.$route.query.id).then(res => {
					if(res.status == 200) {
						if(res.data.code == 200) {
							...
							this.quesPic.push(res.data.data.quesPic.split(",",-1));

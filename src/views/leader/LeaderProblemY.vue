	...
	<div id="examrecord">
		<div class="container">
			...
			<ul class="container_list container_lists ul">
				...
				<li class="overflow">
					<div  v-for="(image, index) in quesPic"  @click="Preview_img(thumbnailPic,index,0)" id="0"><img :src="image" alt="" v-if="image!=''"  @click="Preview_img(thumbnailPic,index,0)" id="0"></div>
				</li>
			</ul>
			<ul class="container_list container_lists ul">
				<p class="van-hairline--bottom exam-title"><img src="../../assets/images/safeQuality/icon_t@2x (5).png" alt="">整改后的现场照片</p>
				<li class="overflow">
					<div  @click="Preview_img(thumbnailUp,index,1)" class="overflow" v-for="(image, index) in thumbnailUp"  id="1"><img :src="image" alt="" v-if="image!=''"  @click="Preview_img(thumbnailUp,index,1)" id="1"></div>
				</li>
			</ul>
			<ul class="container_list container_lists">
				<p class="van-hairline--bottom exam-title"><img src="../../assets/images/safeQuality/icon_t@2x (3).png" alt="">整改人</p>
				<li>
					{{StaffInfoData.processingName}}
				</li>
			</ul>
			<ul class="container_list container_lists">
	...
	export default {
		...
		methods: {
			...
			sumtrienButton() {
				...
				this.a=false;
				if(that.num == 1){
					ajax.postW('/api/safety/updateSafetyReview?id='+that.$route.query.id+'&status='+
					that.num+'&putNameId='+this.StaffInfoData.putNameId+'&putName='+this.StaffInfoData.putName).then(res => {
						if(res.status == 200) {
					...
					ajax.postW('/api/safety/updateSafetyReview?id='+that.$route.query.id+'&status='+
					that.num+'&processOpinion='+that.processOpinion+'&processingId='+this.StaffInfoData.processingId+'&processingName='+this.StaffInfoData.processingName).then(res => {
						if(res.status == 200) {
							if(res.data.code == 200) {
